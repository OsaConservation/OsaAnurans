...
def main():
    ...
    log_cv.to_csv(outdir / "grouped_cv_results.csv", index=False)
    ...
    print("\nValidation:")
    print(pd.DataFrame([val_log]).to_string(index=False))
    ...
    pd.DataFrame([test_log]).to_csv(outdir / "test_metrics.csv", index=False)

    print("\nFINAL HELD-OUT TEST:")
    print(pd.DataFrame([test_log]).to_string(index=False))
